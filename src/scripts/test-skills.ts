 ...
 * the example skills under `<cwd>/.myagent/skills/`.
 *
 * Usage:
 *   cd myagent
...
async function main(): Promise<void> {
  ...
  const okResult = await skillTool.call(
    { skill: "hello-world", args: "myagent" },
  ...
  assert(okText.includes("myagent"), "$ARGUMENTS substituted with \"myagent\"");
  assert(okText.includes("session-test-abc"), "${CLAUDE_SESSION_ID} substituted");
  assert(
    okText.includes(".myagent/skills/hello-world"),
