...
import { getProjectMyAgentDir } from "../utils/paths.js";
...
const work = fs.mkdtempSync(path.join(os.tmpdir(), "myagent-bash-sb-"));
const easyDir = getProjectMyAgentDir(work);
...
async function main(): Promise<void> {
  ...
  const denied = await bashTool.call(
    { command: "echo hijack > /etc/myagent-bash-test 2>&1" },
    { cwd: work },
  );
  ...
  expect("no rogue file landed in /etc", !fs.existsSync("/etc/myagent-bash-test"));
