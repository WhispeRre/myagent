 ...
 *
 * Stubs HOME to a temp dir so anything written under ~/.myagent never
...
async function main(): Promise<void> {
  ...
  await withTempEnv(async (_home, cwd) => {
    ...
    assert(
      tersePrompt.includes("You are myagent"),
  ...
  await withTempEnv(async (home, cwd) => {
    ...
    const written = JSON.parse(
      await fs.readFile(path.join(home, ".myagent", "settings.json"), "utf-8"),
    );
