 ...
 * ~/.myagent. Exits non-zero on any failed assertion.
 */
...
async function main(): Promise<void> {
  // Stub HOME so getMyAgentHome() / state.json land in a sandbox.
  ...
  assert(!fssync.existsSync(path.join(proj, ".myagent", "state.json")), "trust is NOT written into the project dir");
  ...
  const gitignore = await fs.readFile(path.join(wproj, ".myagent", ".gitignore"), "utf-8").catch(() => "");
