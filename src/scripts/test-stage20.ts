...
async function main(): Promise<void> {
...

  {
    const xml = formatTaskNotification({
      ...
      totalTokens: 5000,
      toolUseCount: 7,
      worktreePath: "/repo/.myagent/worktrees/agent-x",
    ...
    assert(xml.includes("<worktree_path>/repo/.myagent/worktrees/agent-x</worktree_path>"), "contains <worktree_path>");
  ...
  await withTempDir(async (repoDir) => {
    ...
    const expectedPath = path.join(repoDir, ".myagent", "worktrees", "agent-x");
    assert(
      worktreePathFor(repoDir, "agent-x") === expectedPath,
      "worktreePathFor gives <repo>/.myagent/worktrees/<slug>",
    );

    // Create a worktree
    const info = await createAgentWorktree("agent-test", repoDir);
    assert(info.gitRoot === repoDir, "createAgentWorktree.gitRoot === repo root");
    assert(info.worktreePath === path.join(repoDir, ".myagent", "worktrees", "agent-test"), "worktreePath matches convention");
  ...
  await withTempDir(async (repoDir) => {
    ...
    // lands in a clean tmp dir rather than the real ~/.myagent/.
    ...
    try {
      ...
      const worktreeDir = path.join(repoDir, ".myagent", "worktrees");
      const entries = await fs.readdir(worktreeDir);
      const created = entries.find((e) => e.startsWith("agent-"));
      assert(!!created, "worktree directory was created under .myagent/worktrees/");
