 ...
 *   MYAGENT_TEAMS=1 npx tsx src/scripts/test-stage21.ts  # with feature flag set
 *
 * The tests that need the feature flag flip `process.env.MYAGENT_TEAMS`
 ...
 * sandboxed `~/.myagent/...`. We point os.homedir() at the temp
...
/**
 * Run `fn` with the MYAGENT_TEAMS env flag in a known state.
...
async function withTeamsFlag<T>(
  ...
  const prevEnv = process.env["MYAGENT_TEAMS"];
  if (enabled) process.env["MYAGENT_TEAMS"] = "1";
  else delete process.env["MYAGENT_TEAMS"];
  try {
    return await fn();
  } finally {
    if (prevEnv !== undefined) process.env["MYAGENT_TEAMS"] = prevEnv;
    else delete process.env["MYAGENT_TEAMS"];
  }
}
...
async function main(): Promise<void> {
  ...
  // ─── [1] Feature flag ──────────────────────────────────────────
  console.log("\n[1] Feature flag — MYAGENT_TEAMS + --agent-teams");
  ...
  await withTeamsFlag(true, async () => {
    assert(isAgentTeamsEnabled() === true, "feature on when MYAGENT_TEAMS=1");
  });
