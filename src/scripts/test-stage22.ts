 ...
 *                              Stop / SubagentStop
 *   [9]  Master kill-switch — MYAGENT_DISABLE_HOOKS
 *
 * Usage:
 *   cd myagent
 *   npx tsx src/scripts/test-stage22.ts
 *
 * Tests stub HOME to a temp dir so written `~/.myagent/settings.json`
...
async function writeUserSettings(home: string, hooks: unknown): Promise<void> {
  const dir = path.join(home, ".myagent");
...
async function main(): Promise<void> {
  ...
  await withTempHome(async (_home, cwd) => {
    ...
    // env var MYAGENT_PROJECT_DIR is exposed
    const envResult = await executeHookCommand({
      hook: { type: "command", command: "printenv MYAGENT_PROJECT_DIR" },
      hookEvent: "PreToolUse",
    ...
    assert(
      ...
      "MYAGENT_PROJECT_DIR env var matches cwd",
    );
  });
  ...
  console.log("\n[9] kill-switch — MYAGENT_DISABLE_HOOKS");
  ...
  await withTempHome(async (home, cwd) => {
    ...
    const prevKill = process.env["MYAGENT_DISABLE_HOOKS"];
    try {
      process.env["MYAGENT_DISABLE_HOOKS"] = "1";
      ...
      if (prevKill !== undefined) process.env["MYAGENT_DISABLE_HOOKS"] = prevKill;
      else delete process.env["MYAGENT_DISABLE_HOOKS"];
    }
  });
