  ...
  inputSchema: {
    ...
    properties: {
      ...
      subagent_type: {
        type: "string",
        description:
          "Which sub-agent definition to use (e.g. 'general-purpose', 'Explore', or a custom name from .myagent/agents/). Defaults to 'general-purpose'.",
      },
      model: {
        type: "string",
        description:
  ...
  async call(input: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    if (name || team_name) {
      if (!isAgentTeamsEnabled()) {
        return {
          content:
            "Error: Agent Teams feature is not enabled. Drop `name` / `team_name`, or restart with --agent-teams (or set MYAGENT_TEAMS=1).",
          isError: true,
        };
      }
      if (!name || !team_name) {
