 ...
 *   1. ~/.myagent/commands/         (user)
 *   2. <cwd>/.myagent/commands/      (project — wins on name clash)
 *
 * Naming:
 *   review.md            → /review
...
export function getUserCommandsDir(): string {
  return getMyAgentPath("commands");
}

/** <cwd>/.myagent/commands */
export function getProjectCommandsDir(cwd: string): string {
  return path.join(getProjectMyAgentDir(cwd), "commands");
}
