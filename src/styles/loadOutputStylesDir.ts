 ...
 *   1. ~/.myagent/output-styles/         (user)
 *   2. <cwd>/.myagent/output-styles/      (project — wins on name clash)
 *
...
/** ~/.myagent/output-styles */
export function getUserOutputStylesDir(): string {
  return getMyAgentPath("output-styles");
}

/** <cwd>/.myagent/output-styles */
export function getProjectOutputStylesDir(cwd: string): string {
  return path.join(getProjectMyAgentDir(cwd), "output-styles");
}
