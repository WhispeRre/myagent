  ...
  if (all.length === 0) {
    yield {
      ...
      message:
        ...
        "  ~/.myagent/skills/<name>/SKILL.md   (user-wide)\n" +
        "  .myagent/skills/<name>/SKILL.md     (project-only)",
    };
    return { handled: true };
  }
  ...
  if (all.length === 0) {
    yield {
      ...
      message:
        ...
        "  ~/.myagent/agents/<name>.md   (user-wide)\n" +
        "  .myagent/agents/<name>.md     (project-only)",
    };
    return { handled: true };
  }
...
export async function* handleHooksCommand(
  ...
  if (report.globallyDisabled) {
    lines.push("⚠ MYAGENT_DISABLE_HOOKS is set — all hooks are disabled this session.");
    lines.push("");
  }
  ...
  lines.push("Set MYAGENT_DISABLE_HOOKS=1 to disable every hook for one session.");
 ...
 * to a text-only listing — myagent doesn't need a full TUI panel for it.
 */
    ...
    if (entries.length === 0) {
      yield {
        ...
        message:
          ...
          "  ~/.myagent/settings.json   (user-wide)\n" +
          "  .myagent/settings.json      (project-only)",
      };
      return { handled: true };
    }
